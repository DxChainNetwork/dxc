...
contract Validators is Base, Initializable, ReentrancyGuard {
    ...
    struct Validator {
        ...
        /// @notice 修改节点分配比例的锁定期，24小时可修改一次
        uint256 rateSettLockingEndBlock;
    }

    /// @notice address => 超级节点信息
    ...
    function initialize(
        ...
        sysRewards.updateEpochWhileElect(totalDeposit, curEpochValidators.length, effictiveValidators.length(), curEpoch);
...
/// @notice Alias of ValidatorProposals in the underlying code
contract Proposals is Base, Initializable, ReentrancyGuard {
    ...
    modifier onlyEffictiveProposal(bytes4 id) {
        require(
            block.number <= proposalInfos[id].initBlock + PROPOSAL_DURATION_EPOCHS * EPOCH_BLOCKS,
    ...
    function initProposal(
        ...
        require(bytes(details).length <= MAX_VALIDATOR_DETAIL_LENGTH, "Proposals: Details is too long");
        require(bytes(name).length <= MAX_VALIDATOR_NAME_LENGTH, "Proposals: name is too long");
        ...
        bytes4 id = bytes4(keccak256(abi.encodePacked(msg.sender, msg.value, rate, name, details, block.number)));
     ...
     * @dev guarantee
     */
    function guarantee(bytes4 id) external nonReentrant onlyEffictiveValidator onlyEffictiveProposal(id) {
    ...
    function updateProposal(
        ...
        require(proposalInfos[id].initBlock != 0, "Proposals: proposal not exist");
        require(proposalInfos[id].proposer == msg.sender, "Proposals: not proposer");
        require(
            proposalInfos[id].status == ProposalStatus.pending,
            "Proposals: The status of proposal must be pending"
        );
