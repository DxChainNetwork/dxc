...
contract NodeVotes is Base, Initializable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    struct RedeemInfo {
        ...
        mapping(uint256 => uint256) epochRedeemVotes;
    }

    struct VotesRewardRedeemInfo {
        address validator;
        string validatorName;
        uint8 validatorRate;
        uint256 validatorTotalVotes;
        uint256 amount;
        uint256 pendingReward;
        uint256 pendingRedeem;
        uint256[] lockRedeemEpochs;
        uint256[] lockRedeemVotes;
    }

    mapping(address => EnumerableSet.AddressSet) voterValidators;
    mapping(address => mapping(address => VoteInfo)) public voteInfos;
    mapping(address => mapping(address => RedeemInfo)) redeemInfos;
    ...
    event LogRedeem(address indexed voter, address indexed validator, uint256 votes);
    event LogEarn(address indexed voter, address indexed validator, uint256 reward);
    event LogRewardTransfer(address indexed from, address indexed to, uint256 amount);
     ...
     * @dev initialize
     */
    function initialize(address _validator, address _sysReward) public onlySystem initializer {
        validators = IValidators(_validator);
        sysRewards = ISystemRewards(_sysReward);
    }

    /**
     * @dev pending vote reward
     */
    function pendingReward(address _val, address _voter) public view returns (uint256) {
        VoteInfo memory voteInfo = voteInfos[_voter][_val];
        uint256 rewardPerVote = sysRewards.getRewardPerVote(_val);

        return ((voteInfo.amount * rewardPerVote) / SAFE_MULTIPLIER) - voteInfo.rewardDebt;
    }

    /**
     * @dev pending redeem votes
     */
    function pendingRedeem(address _val, address _voter) public view returns (uint256) {
        RedeemInfo storage redeemInfo = redeemInfos[_voter][_val];

        (uint256 sumRedeem, uint256 curEpoch) = (0, currentEpoch());

        for (uint256 i = 0; i < redeemInfo.epochs.length(); i++) {
            uint256 redeemEpoch = redeemInfo.epochs.at(i);
            if (curEpoch >= redeemEpoch) {
                sumRedeem += redeemInfo.epochRedeemVotes[redeemEpoch];
            }
        }
        return sumRedeem;
    }
    ...
    /**
     * @dev vote validator count
     */
    function voteListLength(address _voter) external view returns (uint256) {
        return voterValidators[_voter].length();
    }

    function votesRewardRedeemInfo(address _val, address _voter) public view returns (VotesRewardRedeemInfo memory) {
        VotesRewardRedeemInfo memory info;
        info.validator = _val;

        Validators.Validator memory valInfo = validators.validators(_val);
        info.validatorName = valInfo.name;
        info.validatorRate = valInfo.rate;
        info.validatorTotalVotes = valInfo.votes;

        if (!voterValidators[_voter].contains(_val)) return info;

        VoteInfo memory voteInfo = voteInfos[_voter][_val];

        info.amount = voteInfo.amount;
        info.pendingReward = pendingReward(_val, _voter);

        RedeemInfo storage redeemInfo = redeemInfos[_voter][_val];
        (uint256 sumRedeem, uint256 lockRedeemLen, uint256 curEpoch) = (0, 0, currentEpoch());
        ...
        if (sumRedeem > 0) info.pendingRedeem = sumRedeem;

        if (lockRedeemLen != 0) {
            uint256[] memory lockRedeemEpochs = new uint256[](lockRedeemLen);
            uint256[] memory lockRedeemVotes = new uint256[](lockRedeemLen);
            for (uint256 i = 0; i < redeemInfo.epochs.length(); i++) {
                uint256 redeemEpoch = redeemInfo.epochs.at(i);
                if (curEpoch < redeemEpoch) {
                    lockRedeemEpochs[i] = redeemEpoch;
                    lockRedeemVotes[i] = redeemInfo.epochRedeemVotes[redeemEpoch];
                }
            }
            info.lockRedeemEpochs = lockRedeemEpochs;
            info.lockRedeemVotes = lockRedeemVotes;
        }
        return info;
    }

    /**
     * @dev all votesRewardRedeemInfo with page
     */
    function votesRewardRedeemInfoWithPage(
        address _voter,
        uint256 page,
        uint256 size
    ) external view returns (VotesRewardRedeemInfo[] memory) {
        require(page > 0 && size > 0, "NodeVotes: Requests param error");

        uint256 start = (page - 1) * size;
        if (voterValidators[_voter].length() < start) {
            size = 0;
        } else {
            uint256 length = voterValidators[_voter].length() - start;
            if (length < size) {
                size = length;
            }
        }

        VotesRewardRedeemInfo[] memory infos = new VotesRewardRedeemInfo[](size);
        for (uint256 i = 0; i < size; i++) {
            infos[i] = votesRewardRedeemInfo(voterValidators[_voter].at(i + start), _voter);
        }
        return infos;
    }

    function earn(address _val) public nonReentrant {
        VoteInfo storage voteInfo = voteInfos[msg.sender][_val];

        uint256 rewardPerVote = sysRewards.getRewardPerVote(_val);
        uint256 pending = ((voteInfo.amount * rewardPerVote) / SAFE_MULTIPLIER) - voteInfo.rewardDebt;

        if (pending == 0) {
            emit LogEarn(msg.sender, _val, pending);
            return;
        }

        _safeRewardTransfer(pending);

        voteInfo.rewardDebt = (voteInfo.amount * rewardPerVote) / SAFE_MULTIPLIER;
        emit LogEarn(msg.sender, _val, pending);
    }

    function vote(address _val) external payable nonReentrant {
        ...
        require(msg.value >= 1 ether, "NodeVotes: Vote must greater than 1 ether");

        VoteInfo storage voteInfo = voteInfos[msg.sender][_val];
        voterValidators[msg.sender].add(_val);

        uint256 rewardPerVote = sysRewards.getRewardPerVote(_val);

        if (voteInfo.amount > 0) {
            uint256 pending = ((voteInfo.amount * rewardPerVote) / SAFE_MULTIPLIER) - voteInfo.rewardDebt;
            if (pending > 0) {
                _safeRewardTransfer(pending);
                emit LogEarn(msg.sender, _val, pending);
        ...
        voteInfo.amount += msg.value;
        voteInfo.rewardDebt = (voteInfo.amount * rewardPerVote) / SAFE_MULTIPLIER;
        totalVotes += msg.value;
        validators.voteValidator(msg.sender, _val, msg.value);

        emit LogVote(msg.sender, _val, msg.value);
    }

    function redeem(address _val) external nonReentrant {
        require(!Address.isContract(msg.sender), "NodeVotes: The msg.sender can not be contract address");
        _redeem(_val);
    }

    function _redeem(address _val) internal {
        RedeemInfo storage redeemInfo = redeemInfos[msg.sender][_val];

        (uint256 sumRedeem, uint256 curEpoch) = (0, currentEpoch());

        for (uint256 i = 0; i < redeemInfo.epochs.length(); i++) {
            uint256 redeemEpoch = redeemInfo.epochs.at(0);
            if (curEpoch >= redeemEpoch) {
                redeemInfo.epochs.remove(redeemEpoch);
                sumRedeem += redeemInfo.epochRedeemVotes[redeemEpoch];
            }
        }
        if (sumRedeem > 0) {
            payable(msg.sender).transfer(sumRedeem);
            emit LogRedeem(msg.sender, _val, sumRedeem);
        }

        VoteInfo storage voteInfo = voteInfos[msg.sender][_val];
        if (voteInfo.amount == 0 && redeemInfo.epochs.length() == 0) {
            voterValidators[msg.sender].remove(_val);
        }
    }

    function cancelVote(address _val, uint256 _amount) external nonReentrant {
        require(!Address.isContract(msg.sender), "NodeVotes: The msg.sender can not be contract address");

        VoteInfo storage voteInfo = voteInfos[msg.sender][_val];
        require(voterValidators[msg.sender].contains(_val), "NodeVotes: The msg.sender did not vote this validator");
        require(_amount > 0 && voteInfo.amount >= _amount, "NodeVotes: cancel vote amount error");
        ...
        totalVotes -= _amount;
        voteInfo.amount -= _amount;
        voteInfo.rewardDebt = (voteInfo.amount * rewardPerVote) / SAFE_MULTIPLIER;

        uint256 curEpoch = currentEpoch();
        uint256 redeemEpoch = curEpoch + VOTE_CANCEL_EPOCHS;
        redeemInfo.epochs.add(redeemEpoch);
        redeemInfo.epochRedeemVotes[redeemEpoch] += _amount;

        validators.cancelVoteValidator(msg.sender, _val, _amount, voteInfo.amount == 0);

        emit LogCancelVote(msg.sender, _val, _amount);
    }

    function _safeRewardTransfer(uint256 _reward) internal {
        uint256 availableReward = address(this).balance;
        if (_reward > availableReward) {
            payable(msg.sender).transfer(availableReward);
            emit LogRewardTransfer(address(this), msg.sender, availableReward);
        } else {
            payable(msg.sender).transfer(_reward);
            emit LogRewardTransfer(address(this), msg.sender, _reward);
        }
    }
}
