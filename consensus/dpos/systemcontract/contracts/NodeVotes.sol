...
contract NodeVotes is Base, Initializable, ReentrancyGuard {
     ...
     * @dev initialize
     */
    function initialize(address _validator, address _sysReward)
        public
        onlySystem
        initializer
    {
     ...
     */
    function pendingReward(address _val, address _voter)
        public
        view
        returns (uint256)
    {
        ...
        return
            ((voteInfo.amount * rewardPerVote) / SAFE_MULTIPLIER) -
            voteInfo.rewardDebt;
    }
     ...
     */
    function pendingRedeem(address _val, address _voter)
        public
        view
        returns (uint256)
    {
    ...
    function votesRewardRedeemInfo(address _val, address _voter)
        public
        view
        returns (VotesRewardRedeemInfo memory)
    {
        ...
        RedeemInfo storage redeemInfo = redeemInfos[_voter][_val];
        (uint256 sumRedeem, uint256 lockRedeemLen, uint256 curEpoch) = (
            0,
            0,
            currentEpoch()
        );
        ...
        if (lockRedeemLen != 0) {
            ...
            for (uint256 i = 0; i < redeemInfo.epochs.length(); i++) {
                ...
                if (curEpoch < redeemEpoch) {
                    ...
                    lockRedeemVotes[i] = redeemInfo.epochRedeemVotes[
                        redeemEpoch
                    ];
                }
            }
    ...
    function votesRewardRedeemInfoWithPage(
        ...
        VotesRewardRedeemInfo[] memory infos = new VotesRewardRedeemInfo[](
            size
        );
        for (uint256 i = 0; i < size; i++) {
            infos[i] = votesRewardRedeemInfo(
                voterValidators[_voter].at(i + start),
                _voter
            );
        }
        return infos;
    ...
    function earn(address _val) public nonReentrant {
        ...
        uint256 rewardPerVote = sysRewards.getRewardPerVote(_val);
        uint256 pending = ((voteInfo.amount * rewardPerVote) /
            SAFE_MULTIPLIER) - voteInfo.rewardDebt;
    ...
    function vote(address _val) external payable nonReentrant {
        require(
            ...
            "NodeVotes: The msg.sender can not be contract address"
        );
        require(
            validators.isEffictiveValidator(_val),
            "NodeVotes: The val must be validator"
        );
        require(
            msg.value >= 1 ether,
            "NodeVotes: Vote must greater than 1 ether"
        );
    ...
    function cancelVote(address _val, uint256 _amount) external nonReentrant {
        ...
        VoteInfo storage voteInfo = voteInfos[msg.sender][_val];
        require(
            voterValidators[msg.sender].contains(_val),
            "NodeVotes: The msg.sender did not vote this validator"
        );
        require(
            _amount > 0 && voteInfo.amount >= _amount,
            "NodeVotes: cancel vote amount error"
        );
        ...
        validators.cancelVoteValidator(
            msg.sender,
            _val,
            _amount,
            voteInfo.amount == 0
