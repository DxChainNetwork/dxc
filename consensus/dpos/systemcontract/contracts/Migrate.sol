...
contract Migrate is AccessControlEnumerable, Pausable, Initializable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function initialize(
        ...
        _setupRole(EXTERNAL_ADMIN_ROLE, _owner);
    ...
    function accounts(uint256 page, uint256 size) public view returns (address[] memory) {
    ...
    function addAccount(address _addr, uint256 _bal) public onlyRole(EXTERNAL_ADMIN_ROLE) whenNotPaused nonReentrant {
    ...
    function _addAccountsBatch(address[] memory _addrs, uint256[] memory _bals) internal {
        require(_addrs.length == _bals.length, "Migrate: length of addrs is not equal bals");
    ...
    function claim() public nonReentrant whenNotPaused {
        ...
        uint256 _balance = balance[_addr];
        if (isClaimed[_addr] || _balance == 0 || !_accounts.contains(_addr)) return;
