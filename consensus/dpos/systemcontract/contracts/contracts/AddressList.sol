...
// AddressList manages the and black-list addresses
/// @notice Never change the sequence of storage variables.
contract AddressList {
    ...
    address[] blacksFrom;
    address[] blacksTo;
    mapping(address => uint256) blacksFromMap;
    mapping(address => uint256) blacksToMap;

    uint256 public blackLastUpdatedNumber;
    uint256 public rulesLastUpdatedNumber;
    
    // event check rules
    EventCheckRule[] rules;
    mapping(bytes32 => mapping(uint128 => uint256)) rulesMap;
    ...
    function addOrUpdateRule(
        ...
        uint256 old = rulesMap[sig][checkIdx];
        if (old > 0) {
    ...
    function removeRule(bytes32 sig, uint128 checkIdx) external onlyAdmin returns (bool) {
        ...
        uint256 i = rulesMap[sig][checkIdx];
    ...
    function getRuleByIndex(uint32 i)
        external
        view
        returns (
            bytes32,
    ...
    function getRuleByKey(bytes32 sig, uint128 checkIdx)
        external
        view
        returns (
            bytes32,
